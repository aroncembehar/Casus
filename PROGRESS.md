...
**Verification status:** syntax-checked and structurally verified (all four languages' `citations[]` arrays carry matching `key` sets, `<cite>` tags balanced) but **not yet confirmed working in an actual browser** — no headless browser tooling is available in this environment (no Playwright, no Node/npx, no chromium binary), and the user has not granted the Chrome extension's tab-read permission, so live testing via claude-in-chrome was ruled out per their explicit instruction. User is verifying manually at `http://localhost:8934/casus.html` (a local `python3 -m http.server 8934` test server, not part of the deployed project). Test instructions given: open Mangold's Detailed (or Intermediate) card, select text spanning from inside the "p75" sentence across into the "p77" sentence — expect two distinct citation cards (para 74-76 and para 77), not one merged/wrong result.
