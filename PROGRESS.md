...
## Original-12 translation-parity pass (user request, 2026-07-23, inserted ahead of new-case queue)
...
Order (as they appear in `casus.html` / TRACKING.md's remap table): Handelsgesellschaft (done, `6f4fa37`) → Melloni (done, `6cb0661`) → Bavarian Lager (done, `7219779`) → Kadi I (done, `947789b`) → Kadi II (done, `6cd1363`) → Van Gend en Loos (done, `207f557`) → Defrenne (done, `b06f0d9`) → Dominguez (done, `fc71a86`) → Opinion 2/13 (done, `4d22147`) → Stauder → Konstantinidis. One commit per case, same as new-case convention. Once all 12 are at parity, resume the new-case queue below. Also fixed some pre-existing small gaps found along the way: two stale `case-link-pending` spans that should already have been real links (Kadi I → Opinion 2/13), and Opinion 2/13's holistic text had plain-text mentions of Costa v ENEL/Van Gend en Loos instead of jumpToCase links.
...
## Next up after the parity pass (priority queue, in order)
