...
Order (as they appear in `casus.html` / TRACKING.md's remap table): Handelsgesellschaft (done, commit `6f4fa37`) → Melloni → Bavarian Lager → Kadi I → Kadi II → Van Gend en Loos → Defrenne → Dominguez → Opinion 2/13 → Stauder → Konstantinidis. One commit per case, same as new-case convention. Once all 12 are at parity, resume the new-case queue below.
