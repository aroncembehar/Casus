...
Order (as they appear in `casus.html` / TRACKING.md's remap table): Handelsgesellschaft (done, `6f4fa37`) → Melloni (done, `6cb0661`) → Bavarian Lager (done, `7219779`) → Kadi I (done, `947789b`) → Kadi II (done, `6cd1363`) → Van Gend en Loos (done, `207f557`) → Defrenne (done, `b06f0d9`) → Dominguez → Opinion 2/13 → Stauder → Konstantinidis. One commit per case, same as new-case convention. Once all 12 are at parity, resume the new-case queue below. Also fixed two stale `case-link-pending` spans along the way that should already have been real links (Kadi I → Opinion 2/13).
